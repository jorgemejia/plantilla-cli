...
AutoSequelize.prototype.run = function(callback) {
  ...
  var typescriptFiles = [self.options.typescript ? '' : '', ''];

  let memberClassDefinitions = null

  this.build(generateText);

  /**
   * Funcion para generar el archivo del modelo
   * @param err
   */
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      _.each(fields, function(field, i) {
        ...
        tsTableClassDef += tsHelper.def.getMemberClassDefinition(spaces, fieldName, tsVal, tsAllowNull);
      });
      ...
      var tsTableDef = self.options.typescript ? 'export interface ' + tableName + 'Interface {' : '';
      // let  x = tsHelper.def.getMemberDefinition(spaces, fieldName, tsVal, tsAllowNull)

      if(self.options.typescript){
        text[table] = "import { Model, DataTypes } from 'sequelize';\n";
        text[table] += "import { database } from '../config/database';\n\n";
        ...
        text[table] += ` export class ${tableName} extends Model { ${ tsTableClassDef } \n } \n\n`; //"export class " + tableName +" extends Model {}\n\n";
      ...
      _.each(fields, function(field, i){
        ...
        // console.log('Fields attributes', fieldAttr);
        ...
        // console.log('is serial key', isSerialKey);
          ...
          // Set default value on attribute
            ...
            if (self.sequelize.options.dialect === "mssql" &&  defaultVal && defaultVal.toString().toLowerCase() === '(newid())') {
            ...
            //mySql Bit fix
            if (self.tables[table][field].type.toString().toLowerCase() === 'bit(1)') {
              val_text = defaultVal === "b'1'" ? 1 : 0;
            }
            // mssql bit fix
            else if (self.sequelize.options.dialect === "mssql" && self.tables[table][field].type.toString().toLowerCase() === "bit") {
              val_text = defaultVal === "((1))" ? 1 : 0;
            }

            // Time stamps
            if (_.isString(defaultVal)) {
              var field_type = self.tables[table][field].type.toString().toLowerCase();
              if (_.endsWith(defaultVal, '()')) {
                // Descomentar para poner default time stamps
                // val_text = "sequelize.fn('" + defaultVal.replace(/\(\)$/, '') + "')"
              }
              else if (field_type.indexOf('date') === 0 || field_type.indexOf('timestamp') === 0) {
                 if (_.includes(['current_timestamp', 'current_date', 'current_time', 'localtime', 'localtimestamp'], defaultVal.toString().toLowerCase())) {
            ...
            if(defaultVal === null || defaultVal === undefined) {
              ...
              // Validacion para quitar current_timestamp
              if(val_text.includes('current_timestamp')) {
                val_text = "''";
              }
            ...
            var _attr = (self.tables[table][field][attr] || '').toString().toLowerCase();
      ...
      text[table] += spaces + spaces  +"sequelize: database,\n";
...
AutoSequelize.prototype.write = function(attributes, typescriptFiles, callback) {
  ...
  // console.log('dir', self.options.directory)
  // console.log('dir', self.options.interfaceDir)
  ...
  if (self.options.typescript) {
    ...
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      // fs.writeFileSync(path.join(self.options.directory,  'interface.ts'), typescriptFiles[0], 'utf8');
    ...
    tables.forEach((model, index) => {
      let name = model.toString().toLowerCase();
      if (name != "index")
  ...
  function createFile(table, _callback) {
    ...
    // console.log('dir', self.options.interfaceDir)
    var fileName = self.options.camelCaseForFileName ? _.camelCase(table) : table;
    fs.writeFile(path.resolve(path.join(self.options.directory, fileName + (self.options.typescript ? '.model.ts' : '.js'))), attributes[table], _callback);
    if (self.options.typescript) {
      if (typescriptFiles !== null && typescriptFiles.length > 1) {
        if(!fs.existsSync(self.options.interfaceDir)) {
          fs.mkdirSync(self.options.interfaceDir);
        }
        fs.writeFileSync(path.join(self.options.interfaceDir,  fileName + '.interface.ts'), typescriptFiles[0], 'utf8');
      }
    }
  }
};
